...
import net.proselyte.jwtappdemo.service.MailSenderService;
...
public class UserServiceImpl implements UserService {	
    ...
    public UserServiceImpl(UserRepository userRepository, RoleRepository roleRepository,
    		BCryptPasswordEncoder passwordEncoder, MailSenderService mailSender) {
    ...
    @Override
    public void sendMessage(User currentUser, List<Message> messages) {
    	if (!StringUtils.isEmpty(currentUser.getEmail())) {
        	final String subject = "10 last messages";        	
        	final String text = messages.stream()
        	      .map(n -> String.valueOf(n))
        	      .collect(Collectors.joining(";    ", "{", "}"));        	 
        	   // System.out.println(result);        	
        	
            mailSender.send(currentUser.getEmail(), subject, text);
        }
    }
}
